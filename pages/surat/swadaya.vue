...
const groupedKegiatan = {}
formSwadaya.kegiatan.forEach((item) => {
  const month = getMonthName(item.tanggal)
  if (!groupedKegiatan[month]) groupedKegiatan[month] = []
  groupedKegiatan[month].push(item)
})
const generatePDFBlob = () => {
  const doc = new jsPDF('landscape', 'mm', 'a4')
  const pageWidth = doc.internal.pageSize.getWidth()
  ...
  row1Headers.forEach((header, i) => {
    ...
    const height = (i === 8) ? 20 : 10

    if (header) {
      doc.text(header, currentX + width / 2, startY + 6, { align: 'center' })
    }
    doc.rect(currentX, startY, width, height)
    currentX += width
  })

  // Row 2 → BIDANG
  currentX = startX + colWidths[0] + colWidths[1] + colWidths[2]
  startY += 10
  doc.setFontSize(6.5)
  ...
  Object.entries(groupedKegiatan).forEach(([month, group]) => {
    // Month label row
    currentX = startX
    const monthRowValues = ['', month, '', ...Array(5).fill(''), '']
    doc.setFont('Times', 'bold')
    monthRowValues.forEach((val, i) => {
      const align = i === 1 ? 'left' : 'center'
      const textX = align === 'center' ? currentX + colWidths[i] / 2 : currentX + 1
    ...
    group.forEach((k, idx) => {
      ...
      textLines.forEach((lines, i) => {
        ...
        let align = 'center'
        if (i === 2) align = 'left'
        else if (i >= 3 && i <= 7) align = 'right'
        else if (i === 8) align = 'right'
    ...
    // ==== TOTAL per BULAN ====
    const subtotal = group.reduce((sum, k) => sum + (parseInt(k.jumlah) || 0), 0)
    const totalPerBidangInMonth = bidangList.map(b => {
      return group
        .filter(k => k.bidang === b)
    ...
    currentX = startX
    const monthTotalRow = ['', '', 'TOTAL', ...totalPerBidangInMonth.map(n => formatRupiah(n)), formatRupiah(subtotal)]
    doc.setFont('Times', 'bold')
    monthTotalRow.forEach((val, i) => {
      const align = (i >= 3 && i <= 7) || i === 8 ? 'right' : 'left'
      const textX = align === 'center' ? currentX + colWidths[i] / 2 : currentX + 1
      doc.text(val || '', textX, currentY + 6, { align })
      doc.rect(currentX, currentY, colWidths[i], 10)
      currentX += colWidths[i]
...
<template>
  <div class="max-w-6xl mx-auto p-6 space-y-8">
    ...
    <!-- Preview -->
    <section class="bg-white p-6 rounded shadow border laporan-preview">
